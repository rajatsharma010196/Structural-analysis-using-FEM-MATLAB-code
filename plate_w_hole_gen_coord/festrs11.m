...
%collecting nodes of faces AB and ED
nodes_egde_AB = [] ;
nodes_egde_ED = [] ;

for i = 1:nno
    if abs(Xn(i,1)) <= eps % if x coordinate is zero
       nodes_egde_AB = [nodes_egde_AB ; i];
    end
end

for i = 1:nno
    if abs(Xn(i,2)) <= eps  % if y coordinate is zero
        nodes_egde_ED = [nodes_egde_ED  ; i];
    end
end

%Printing the stresses on nodes of face AB AND ED
fprintf('\nstresses on face AB :: \n');
fprintf('sigma_xx\t\t\tsigma_yy\t\ttau_xy\t\t\tsigma_zz\t\t\ttau_yz\t\t\ttau_xz\n');
for i = 1:size(nodes_egde_AB)
fprintf('%f\t\t',  stress_at_nodes(nodes_egde_AB(i),:));
fprintf('\n');
end

fprintf('\n\nstresses on face ED :: \n');
fprintf('sigma_xx\t\t\tsigma_yy\t\ttau_xy\t\t\tsigma_zz\t\t\ttau_yz\t\t\ttau_xz\n');
for i = 1:size(nodes_egde_ED)
fprintf('%f\t\t',  stress_at_nodes(nodes_egde_ED(i),:));
fprintf('\n');
end
